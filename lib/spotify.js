...
SpotifyWebApi.prototype.searchArtists = function(artist) {
  ...
  return this.request(uri).then(function(response) {
    if (response && response.artists && response.artists.items && response.artists.items[0]) {
      return Promise.resolve({ body: response });
    } else {
      return Promise.reject(response);
    }
...
SpotifyWebApi.prototype.searchAlbums = function(album, artist) {
  ...
  return this.request(uri).then(function(response) {
    if (response && response.albums && response.albums.items && response.albums.items[0]) {
      return Promise.resolve({ body: response });
    } else {
      return Promise.reject(response);
    }
...
SpotifyWebApi.prototype.searchPlaylists = function(playlist) {
  ...
  return this.request(uri).then(function(response) {
    if (response.playlists && response.playlists.items && response.playlists.items[0]) {
      return Promise.resolve({ body: response });
    } else {
      return Promise.reject(response);
    }
