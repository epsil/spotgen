...
Collection.prototype.toLog = function () {
  ...
  this.forEachTrack(function (track) {
    if (track.title) {
      ...
      var popularity = Number.isInteger(track.popularity)
      var lastfm = Number.isInteger(track.lastfm)
      if (popularity || lastfm) {
        line += ' ('
        line += popularity ? ('Spotify popularity: ' + track.popularity) : ''
        line += (popularity && lastfm) ? ', ' : ''
        line += lastfm ? 'Last.fm rating: ' + track.lastfm : ''
        line += ')'
      }
      result += line + '\n'
    }
