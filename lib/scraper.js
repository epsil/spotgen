  ...
  function getPages (nextUri, result, count) {
    ...
    return http(nextUri).then(function (data) {
      ...
      if (uri.match(/\/\+tracks/gi)) {
        ...
        // list of albums
        html.find('td.chartlist-name').each(function () {
          lines += '#album ' + util.trim($(this).text()) + '\n'
        })
      } else {
        // list of tracks by various artists
        html.find('td.chartlist-name').each(function () {
          lines += util.trim($(this).text()) + '\n'
        })
      }
  ...
  function getPages (nextUri, result, count) {
    ...
    return http(nextUri).then(function (data) {
      ...
      html.find('div.artist-work').each(function () {
        var artist = util.trim($(this).find('ul.artist-list li:first').text())
        var album = util.trim($(this).find('h2.work-title').text())
  ...
  function getPages (nextUri, result, count) {
    ...
    return http(nextUri).then(function (data) {
      ...
      html.find('div.chart_details').each(function () {
        var artist = util.trim($(this).find('a.artist').text())
        var album = util.trim($(this).find('a.album').text())
        lines += '#album ' + artist + ' - ' + album + '\n'
      })
      console.log(lines.trim() + '\n')
      result += lines
      ...
      if (uri.match(/\/comments\//gi)) {
        ...
        html.find('div.entry div.md').each(function () {
          ...
          if (sentences.length > 1) {
            lines += util.cleanup(sentences[0]) + '\n'
            return
          }
          ...
          if (lines.length > 1) {
            lines += util.cleanup(lines[0]) + '\n'
            return
          }
          // fall-back case
          lines += util.cleanup(body) + '\n'
        })
      } else {
        // post listing
        html.find('a.title').each(function () {
          var track = util.cleanup($(this).text())
          lines += track + '\n'
        })
      }
      console.log(lines.trim() + '\n')
