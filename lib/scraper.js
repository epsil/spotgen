  ...
  function getPages (nextUri, result, count) {
    ...
    return http(nextUri).then(function (data) {
      ...
      if (uri.match(/\/\+tracks/gi)) {
        ...
        // NOTE: did not get the chance to test this code
        // but that's how last.fm is now
        // representing the entries in its chartlists 
        html.find('tr.chartlist-row').each(function () {
          var title = $(this).find('chartlist-name').textContent.trim()
          var artist = $(this).find('chartlist-artist').textContent.trim()
          lines += util.normalize(title+' '+artist) + '\n'
        })
      }
